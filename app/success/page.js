...
function SuccessContent() {
  ...
  useEffect(() => {
    // Disparar evento de compra (Purchase) para Meta Pixel
    if (typeof window !== 'undefined' && window.fbq) {
      ...
      // Añadir valor y moneda si están disponibles en los parámetros
      if (price) {
        purchaseData.value = parseFloat(price);
        purchaseData.currency = currency;
      }
      
      window.fbq('track', 'Purchase', purchaseData);
    }
  }, [orderId, price, currency]);
